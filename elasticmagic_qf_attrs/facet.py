...
from .facet_result import AttrFacetFilterResult, AttrRangeFacetFilterResult, \
    AttrRangeFacet
...
class AttrRangeFacetFilter(AttrRangeSimpleFilter):
    ...
    def _process_result(
        ...
        for selected_attr_id, w in self._iter_attr_values(params):
            if (
                    self._parse_last_value(w, 'gte') or
                    self._parse_last_value(w, 'lte')
            ):
        ...
        for agg_name, agg_res in result.aggregations.items():
            if not agg_name.startswith(f'{self._agg_name()}:'):
                continue
            parsed_attr_id = _parse_attr_id_from_agg_name(agg_name)
            if parsed_attr_id is None:
                continue
            facet_result.add_facet(
                AttrRangeFacet(
                    parsed_attr_id,
                    agg_res.doc_count,
                    parsed_attr_id in selected_attr_ids
                )
            )
